...
The same dog-eared note the desktop draws in `crates/ymemo-desktop/src/icon.rs`, in three
...
- `res/mipmap-*/ic_launcher.png` — API 24-25, generated by `tool/gen_launcher_icons.py` from
  the same geometry. Re-run it after changing the vector.
- `tool/gen_widget_previews.py` draws the widget-picker previews for the same reason:
