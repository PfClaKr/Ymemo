...
fn main() -> Result<()> {
    ...
    // (vanilla GNOME, and the reason `Ctx::has_tray` exists) a hidden start would be an app
    // with no window and no icon, running an unlocked-vault timer nobody can reach.
    let stay_hidden = autostart::launched_hidden() && ctx.has_tray.get();
    // Do not raise the lock window when the session already unlocked us.
    if !auto_unlocked && !stay_hidden {
